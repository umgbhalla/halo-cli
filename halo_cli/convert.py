"""Convert raw Langfuse observation JSONL into HALO OTLP-shaped span JSONL."""
...
def convert(
    ...
    *,
    ...
    return {"source": str(source), "output": str(output), "trace_count": len(trace_ids), **counts}


def convert_command(
    source: Path = typer.Argument(..., exists=True, readable=True, dir_okay=False, help="Raw Langfuse JSONL."),
    output: Path = typer.Argument(..., dir_okay=False, help="Output HALO span JSONL."),
    start: str | None = typer.Option(None, "--start", help="Inclusive UTC start, e.g. 2026-05-01T00:00:00Z."),
    end: str | None = typer.Option(None, "--end", help="Exclusive UTC end."),
    require_completed_traces: bool = typer.Option(
        False, "--require-completed-traces", help="Only keep traces whose observations all have endTime."
    ),
) -> None:
    """Convert raw Langfuse observations to HALO span JSONL."""
    summary = convert(
        source,
        output,
        start=parse_time(start),
        end=parse_time(end),
        require_completed_traces=require_completed_traces,
    )
